...
class Snake {
    constructor(x, y, name, isPlayer = false) {
        ...
        this.name = name;
        this.faceEmoji = isPlayer ? "👩" : (Math.random() > 0.5 ? "👱‍♀️" : "👩‍🦰");
        ...
        this.color = isPlayer ? "#ff2a6d" : botColors[Math.floor(Math.random() * botColors.length)];
...
function initGame(pName) {
    ...
    player = new Snake(GAME_WIDTH/2, GAME_HEIGHT/2, nome, true);
    bots = [];
    foods = [];
    particles = [];
    mouse.active = false;
...
startBtn.addEventListener('click', () => {
    initGame(playerNameInput.value);
});
